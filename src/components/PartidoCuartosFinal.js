...
const PartidoCuartosFinal = () => {
    ...
    const cargarPartidosYCompletar = useCallback(async () => {
        ...
        if (partidosList.length >= 8) {
        setPartidos(partidosList.slice(0, 8));
        return;
        }
        ...
        for (let j = i + 1; j < equiposList.length; j++) {
            ...
            const clave = `${id1}-${id2}`;
            if (!yaUsados.has(clave) && combinaciones.length + partidosList.length < 8) {
            combinaciones.push({
...
export default PartidoCuartosFinal;
